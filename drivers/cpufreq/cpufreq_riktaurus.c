/*
 *  drivers/cpufreq/cpufreq_riktaurus.c
 *
 *   Editor Rittik Bhowmik and Team Taurus @XDA <rittik.manish@gmail.com>
 *
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_RIKTAURUS
static
#endif
struct cpufreq_governor cpufreq_gov_riktaurus = {
	.name                   = "TaurusR",
...
static void riktaurus_init_cpu(int cpu)
{
...
{
	printk_once(KERN_INFO "CPUFREQ: riktaurus sampling_rate_max "
...
/* cpufreq_riktaurus Governor Tunables */
...
#define DEPRECATION_MSG(file_name)					\
	printk_once(KERN_INFO "CPUFREQ: Per core riktaurus sysfs "	\
...
static void cpufreq_riktaurus_timer(unsigned long data)
{
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	for_each_cpu(j, policy->cpus) {
		...
		/*
		 * For the purpose of riktaurus, waiting for disk IO is an
...
static void cpufreq_riktaurus_idle(void)
{
	int i;
				   ...
				   unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		pm_idle_old = pm_idle;
		pm_idle = cpufreq_riktaurus_idle;
		break;
...
static inline void cpufreq_riktaurus_update_time(void)
{
	struct cpu_dbs_info_s *this_dbs_info;
	struct cpufreq_policy *policy;
	int j;
...
static int cpufreq_riktaurus_up_task(void *data)
{
	unsigned long flags;
	struct cpu_dbs_info_s *this_dbs_info;
	struct cpufreq_policy *policy;
...
static void cpufreq_riktaurus_freq_down(struct work_struct *work)
{
	unsigned long flags;
	struct cpu_dbs_info_s *this_dbs_info;
	struct cpufreq_policy *policy;
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	cpu_timer.function = cpufreq_riktaurus_timer;

	up_task = kthread_create(cpufreq_riktaurus_up_task, NULL,
			"kadaptiveup");
	...
	INIT_WORK(&freq_scale_down_work, cpufreq_riktaurus_freq_down);


	return cpufreq_register_governor(&cpufreq_gov_riktaurus);
err_freeuptask:
...
{
	cpufreq_unregister_governor(&cpufreq_gov_riktaurus);
}


MODULE_AUTHOR("Venkatesh Pallipadi <venkatesh.pallipadi@intel.com>");
MODULE_AUTHOR("Alexey Starikovskiy <alexey.y.starikovskiy@intel.com>");
MODULE_DESCRIPTION("'cpufreq_riktaurus' - A dynamic cpufreq governor for "
	"Low Latency Frequency Transition capable processors");
MODULE_LICENSE("GPL");

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_RIKTAURUS
